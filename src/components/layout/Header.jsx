import React, { useState } from 'react';
    ...
    import { ShieldCheck, ShoppingCart, X, Plus, Minus, Trash2, Menu } from 'lucide-react';
    ...
    const CartSidebar = () => {
        ...
        const handleWhatsAppInquiry = () => {
            ...
            const whatsappURL = isMobile
                ? `whatsapp://send?phone=${phone}&text=${fullMessage}`
                : `https://web.whatsapp.com/send?phone=${phone}&text=${fullMessage}`;

            window.open(whatsappURL, "_blank");
        };

        return (
            <SheetContent className="bg-card border-l border-border/50 flex flex-col w-full sm:max-w-lg">
                <SheetHeader>
                ...
                {cartItems.length > 0 ? (
                    ...
                    <div className="flex-grow flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-muted-foreground mt-2">Agrega productos para verlos aquí.</p>
                    </div>
                )}
            </SheetContent>
        );
        ...
        return (
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        <div className="flex flex-col items-center justify-center h-full pt-20 overflow-y-auto">
                            {itemsToRender.map((item, index) => (
                                ...
                                >
                                    <Link
                                        ...
                                        className="text-2xl font-bold text-gray-200 hover:text-cyan-400 transition-colors py-3 block"
                                    >
                                        {item.name}
                                    </Link>
                                </motion.div>
    ...
    const Header = () => {
      ...
      const isProducts = location.pathname.startsWith('/category');
      ...
      return (
        <Sheet>
            <header className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-lg border-b border-border/50">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-20">
                        <Link to="/" className="flex-shrink-0 z-50" onClick={() => isMobileMenuOpen && setIsMobileMenuOpen(false)}>
                            <motion.div
                        ...
                        <nav className="hidden md:flex flex-1 justify-center items-center h-full">
                            {isProducts ? (
                                ...
                                <div className="flex items-center space-x-8">
                                    {navLinks.map((item) => (
                                       <Link key={item.name} to={item.path} className="text-sm font-medium text-gray-300 hover:text-white transition-colors cursor-pointer">{item.name}</Link>
                                    ))}
                                </div>
                            )}
                        </nav>
                            ...
                            >
                                <SheetTrigger asChild>
                                    <Button variant="ghost" size="icon" className="relative">
                                        <ShoppingCart className="h-6 w-6 text-gray-300 hover:text-cyan-400 transition-colors" />
                                        {totalItems > 0 && (
            ...
            <CartSidebar />
            <MobileMenu isOpen={isMobileMenuOpen} setIsOpen={setIsMobileMenuOpen} isProductsPage={isProducts} categories={categories} />
        </Sheet>
      );
    };
